...
def test_key_conditions_partition_one(test_table_sn_with_sorted_partition):
    ...
    with pytest.raises(ClientError, match="ValidationException.*EQ"):
        full_query(
            table,
            KeyConditions={"p": {"AttributeValueList": [], "ComparisonOperator": "EQ"}},
        )
    with pytest.raises(ClientError, match="ValidationException.*EQ"):
        full_query(
            table,
...
def test_begins_with_more_str(dynamodb, test_table_ss):
    p = random_string()
    items = [
        {"p": p, "c": sort_key, "str": "a"} for sort_key in ["ÿÿÿ", "cÿbÿ", "cÿbÿÿabg"]
    ]
    ...
    got_items = full_query(
        ...
        KeyConditions={
            "p": {"AttributeValueList": [p], "ComparisonOperator": "EQ"},
            "c": {"AttributeValueList": ["ÿÿ"], "ComparisonOperator": "BEGINS_WITH"},
        },
    )
    assert sorted([d["c"] for d in got_items]) == sorted(
        [d["c"] for d in items if d["c"].startswith("ÿÿ")]
    )
    ...
    got_items = full_query(
        ...
        KeyConditions={
            "p": {"AttributeValueList": [p], "ComparisonOperator": "EQ"},
            "c": {"AttributeValueList": ["cÿbÿ"], "ComparisonOperator": "BEGINS_WITH"},
        },
    )
    assert sorted([d["c"] for d in got_items]) == sorted(
        [d["c"] for d in items if d["c"].startswith("cÿbÿ")]
    )
...
def test_key_conditions_bytes_eq(test_table_sb_with_sorted_partition):
    ...
    got_items = full_query(
        ...
        KeyConditions={
            "p": {"AttributeValueList": [p], "ComparisonOperator": "EQ"},
            "c": {
                "AttributeValueList": [bytearray("003", "ascii")],
...
def test_key_conditions_bytes_lt(test_table_sb_with_sorted_partition):
    ...
    got_items = full_query(
        ...
        KeyConditions={
            "p": {"AttributeValueList": [p], "ComparisonOperator": "EQ"},
            "c": {
                "AttributeValueList": [bytearray("005", "ascii")],
...
def test_key_conditions_bytes_begins_with(test_table_sb_with_sorted_partition):
    ...
    got_items = full_query(
        ...
        KeyConditions={
            "p": {"AttributeValueList": [p], "ComparisonOperator": "EQ"},
            "c": {
                "AttributeValueList": [bytearray("00", "ascii")],
...
def test_key_conditions_bytes_begins_with_ff(dynamodb, test_table_sb):
    ...
    got_items = full_query(
        ...
        KeyConditions={
            "p": {"AttributeValueList": [p], "ComparisonOperator": "EQ"},
            "c": {
                "AttributeValueList": [bytearray([255])],
                "ComparisonOperator": "BEGINS_WITH",
            },
        },
    )
...
def test_key_conditions_unsigned_bytes(test_table_sb):
    ...
    got_items = full_query(
        ...
        KeyConditions={
            "p": {"AttributeValueList": [p], "ComparisonOperator": "EQ"},
            "c": {
                "AttributeValueList": [bytearray([127]), bytearray([128])],
                "ComparisonOperator": "BETWEEN",
            },
        },
    )
...
def test_query_basic_restrictions(dynamodb, filled_test_table):
    ...
    print(got_items)
    assert multiset([item for item in items if item["p"] == "long"]) == multiset(
        got_items
    )
    ...
    print(got_items)
    assert multiset(
        [item for item in items if item["p"] == "long" and item["c"] <= "14"]
    ) == multiset(got_items)
    ...
    print(got_items)
    assert multiset(
        [item for item in items if item["p"] == "long" and item["c"] > "15"]
    ) == multiset(got_items)
    ...
    assert multiset(
        [
            item
            for item in items
            if item["p"] == "long" and item["c"] >= "155" and item["c"] <= "164"
        ]
    ) == multiset(got_items)
    ...
    for page in paginator.paginate(
        ...
        print(
            [
                item
                for item in items
                if item["p"] == "long" and item["c"].startswith("11")
            ]
        )
        got_items += page["Items"]
    print(got_items)
    assert multiset(
        [item for item in items if item["p"] == "long" and item["c"].startswith("11")]
    ) == multiset(got_items)
