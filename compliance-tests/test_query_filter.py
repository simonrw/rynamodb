        ...
        for xn in ["l", "m", "ns", "bool"]:
            ...
            with pytest.raises(ClientError, match="ValidationException.* type"):
                full_query(
                    ...
                    QueryFilter={
                        xn: {"AttributeValueList": [xv], "ComparisonOperator": op}
                    },
                )
...
def test_query_filter_between(test_table_sn_with_data):
    ...
    for xn in ["i", "s", "b"]:
        ...
        got_items = full_query(
            ...
            QueryFilter={
                xn: {"AttributeValueList": [xv1, xv2], "ComparisonOperator": "BETWEEN"}
            },
        )
...
def test_query_filter_num_between_reverse(test_table_sn_with_data):
    ...
    with pytest.raises(ClientError, match="ValidationException.* BETWEEN"):
        full_query(
            ...
            QueryFilter={
                "i": {"AttributeValueList": [i1, i2], "ComparisonOperator": "BETWEEN"}
            },
        )
...
def test_query_filter_between_different_types(test_table_sn_with_data):
    ...
    with pytest.raises(ClientError, match="ValidationException.* type"):
        full_query(
            ...
            QueryFilter={
                "i": {"AttributeValueList": ["a", 3], "ComparisonOperator": "BETWEEN"}
            },
        )
    ...
    for params in [[], [2], [2, 3, 4]]:
        with pytest.raises(ClientError, match="ValidationException.*BETWEEN"):
            full_query(
                ...
                QueryFilter={
                    "i": {"AttributeValueList": params, "ComparisonOperator": "BETWEEN"}
                },
            )
...
def test_query_filter_in(test_table_sn_with_data):
    ...
    for xn in ["i", "s", "b"]:
        ...
        got_items = full_query(
            ...
            QueryFilter={
                xn: {"AttributeValueList": [xv1, xv2, xv3], "ComparisonOperator": "IN"}
            },
        )
        expected_items = [
            item
...
def test_query_filter_in_different_types(test_table_sn_with_data):
    ...
    with pytest.raises(ClientError, match="ValidationException.* type"):
        full_query(
            ...
            QueryFilter={
                "i": {"AttributeValueList": ["a", 3], "ComparisonOperator": "IN"}
            },
        )
...
def test_query_filter_begins(test_table_sn_with_data):
    ...
    for xn in ["s", "b"]:
        ...
        got_items = full_query(
            ...
            QueryFilter={
                xn: {"AttributeValueList": [start], "ComparisonOperator": "BEGINS_WITH"}
            },
        )
    ...
    for op in ["CONTAINS", "NOT_CONTAINS"]:
        ...
        for xn in ["s", "b"]:
            ...
            got_items = full_query(
                ...
                QueryFilter={
                    xn: {"AttributeValueList": [substring], "ComparisonOperator": op}
                },
            )
            if op == "CONTAINS":
...
def test_query_filter_empty_results(test_table_sn_with_data):
    ...
    got_items = full_query(
        ...
        KeyConditions={"p": {"AttributeValueList": [p], "ComparisonOperator": "EQ"}},
        QueryFilter={
            "non_existent": {"AttributeValueList": [3], "ComparisonOperator": "EQ"}
        },
    )
    assert got_items == []
...
def test_query_filter_and(test_table_sn_with_data):
    ...
    got_items = full_query(
        ...
        KeyConditions={"p": {"AttributeValueList": [p], "ComparisonOperator": "EQ"}},
        QueryFilter={
            "i": {"AttributeValueList": [i], "ComparisonOperator": "EQ"},
...
def test_query_filter_invalid_conditional_operator(test_table_sn_with_data):
    ...
    for conditional_operator in ["DOG", "and"]:
        with pytest.raises(
            ClientError, match="ValidationException.*" + conditional_operator
        ):
            full_query(
                table,
                KeyConditions={
                ...
                QueryFilter={
                    "i": {"AttributeValueList": [1], "ComparisonOperator": "EQ"},
...
def test_query_filter_and_attributes_to_get(test_table):
    ...
    got_items = full_query(
        ...
        KeyConditions={"p": {"AttributeValueList": [p], "ComparisonOperator": "EQ"}},
        QueryFilter={
            "x": {"AttributeValueList": ["mouse"], "ComparisonOperator": "EQ"}
        },
        AttributesToGet=["y"],
    )
    # Note that:
