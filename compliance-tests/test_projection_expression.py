...
def test_projection_expression_toplevel_syntax(test_table_s):
    ...
    assert test_table_s.get_item(
        Key={"p": p}, ConsistentRead=True, ProjectionExpression="a"
    )["Item"] == {"a": "hello"}
    assert test_table_s.get_item(
        Key={"p": p},
        ConsistentRead=True,
        ProjectionExpression="#name",
        ExpressionAttributeNames={"#name": "a"},
    )["Item"] == {"a": "hello"}
    assert test_table_s.get_item(
        Key={"p": p}, ConsistentRead=True, ProjectionExpression="a,b"
    )["Item"] == {"a": "hello", "b": "hi"}
    assert test_table_s.get_item(
        Key={"p": p}, ConsistentRead=True, ProjectionExpression=" a  ,   b  "
    )["Item"] == {"a": "hello", "b": "hi"}
    ...
    with pytest.raises(ClientError, match="ValidationException"):
        test_table_s.get_item(
            ...
            ConsistentRead=True,
            ProjectionExpression="#name",
            ExpressionAttributeNames={"#wrong": "a"},
        )["Item"] == {"a": "hello"}
    with pytest.raises(ClientError, match="ValidationException"):
        test_table_s.get_item(
            Key={"p": p},
            ConsistentRead=True,
            ProjectionExpression="#name",
            ExpressionAttributeNames={"#name": "a", "#unused": "b"},
        )["Item"] == {"a": "hello"}
    ...
    with pytest.raises(ClientError, match="ValidationException"):
        test_table_s.get_item(
            Key={"p": p}, ConsistentRead=True, ProjectionExpression="a,a"
        )["Item"]
    ...
    with pytest.raises(ClientError, match="ValidationException"):
        test_table_s.get_item(
            Key={"p": p}, ConsistentRead=True, ProjectionExpression=""
        )["Item"]
...
def test_projection_expression_query(test_table):
    p = random_string()
    items = [
        {"p": p, "c": str(i), "a": str(i * 10), "b": str(i * 100)} for i in range(10)
    ]
    with test_table.batch_writer() as batch:
        for item in items:
            batch.put_item(item)
    for wanted in [
        ["a"],  # only non-key attributes
        ["c", "a"],  # a key attribute (sort key) and non-key
        ["p", "c"],  # entire key
        ["nonexistent"],  # none of the items have this attribute!
    ]:
...
def test_projection_expression_path(test_table_s):
    ...
    test_table_s.put_item(
        Item={"p": p, "a": {"b": [2, 4, {"x": "hi", "y": "yo"}], "c": 5}, "b": "hello"}
    )
    # Fetching the entire nested document "a" works, of course:
    assert test_table_s.get_item(
        Key={"p": p}, ConsistentRead=True, ProjectionExpression="a"
    )["Item"] == {"a": {"b": [2, 4, {"x": "hi", "y": "yo"}], "c": 5}}
    # If we fetch a.b, we get only the content of b - but it's still inside
    # the a dictionary:
    assert test_table_s.get_item(
        Key={"p": p}, ConsistentRead=True, ProjectionExpression="a.b"
    )["Item"] == {"a": {"b": [2, 4, {"x": "hi", "y": "yo"}]}}
    # Similarly, fetching a.b[0] gives us a one-element array in a dictionary.
    # Note that [0] is the first element of an array.
    assert test_table_s.get_item(
        Key={"p": p}, ConsistentRead=True, ProjectionExpression="a.b[0]"
    )["Item"] == {"a": {"b": [2]}}
    assert test_table_s.get_item(
        Key={"p": p}, ConsistentRead=True, ProjectionExpression="a.b[2]"
    )["Item"] == {"a": {"b": [{"x": "hi", "y": "yo"}]}}
    assert test_table_s.get_item(
        Key={"p": p}, ConsistentRead=True, ProjectionExpression="a.b[2].y"
    )["Item"] == {"a": {"b": [{"y": "yo"}]}}
    ...
    assert (
        test_table_s.get_item(
            Key={"p": p}, ConsistentRead=True, ProjectionExpression="a.b[3]"
        )["Item"]
        == {}
    )
    assert (
    ...
    assert (
        test_table_s.get_item(
            Key={"p": p}, ConsistentRead=True, ProjectionExpression="a.x.y"
        )["Item"]
        == {}
    )
    assert (
        test_table_s.get_item(
            Key={"p": p}, ConsistentRead=True, ProjectionExpression="a.b[3].x"
        )["Item"]
        == {}
    )
    ...
    assert (
        test_table_s.get_item(
            Key={"p": p}, ConsistentRead=True, ProjectionExpression="a.b.x"
        )["Item"]
        == {}
    )
    assert (
    ...
    assert (
        test_table_s.get_item(
            Key={"p": p}, ConsistentRead=True, ProjectionExpression="a.b[0].x"
        )["Item"]
        == {}
    )
    assert (
        test_table_s.get_item(
            Key={"p": p}, ConsistentRead=True, ProjectionExpression="a.b[0][0]"
        )["Item"]
        == {}
    )
    ...
    assert test_table_s.get_item(
        Key={"p": p}, ConsistentRead=True, ProjectionExpression="a.b[0],a.b[1]"
    )["Item"] == {"a": {"b": [2, 4]}}
    assert test_table_s.get_item(
        Key={"p": p}, ConsistentRead=True, ProjectionExpression="a.b[0],a.c"
    )["Item"] == {"a": {"b": [2], "c": 5}}
    assert test_table_s.get_item(
        Key={"p": p}, ConsistentRead=True, ProjectionExpression="a.c,b"
    )["Item"] == {"a": {"c": 5}, "b": "hello"}
    ...
    assert test_table_s.get_item(
        Key={"p": p},
        ConsistentRead=True,
        ProjectionExpression="a.x, a.b[0], x, a.b[3].x",
    )["Item"] == {"a": {"b": [2]}}
    ...
    with pytest.raises(ClientError, match="ValidationException"):
        test_table_s.get_item(
            Key={"p": p}, ConsistentRead=True, ProjectionExpression="a.b[0],a.b[0]"
        )["Item"]
    ...
    with pytest.raises(ClientError, match="ValidationException"):
        test_table_s.get_item(
            Key={"p": p}, ConsistentRead=True, ProjectionExpression="a,a.b[0]"
        )["Item"]
...
def test_projection_expression_path_references(test_table_s):
    ...
    assert test_table_s.get_item(
        Key={"p": p},
        ConsistentRead=True,
        ProjectionExpression="#n1.b",
        ExpressionAttributeNames={"#n1": "a"},
    )["Item"] == {"a": {"b": 1}}
    assert test_table_s.get_item(
        Key={"p": p},
        ConsistentRead=True,
        ProjectionExpression="a.#n2",
        ExpressionAttributeNames={"#n2": "b"},
    )["Item"] == {"a": {"b": 1}}
    assert test_table_s.get_item(
        Key={"p": p},
        ConsistentRead=True,
        ProjectionExpression="#n1.#n2",
        ExpressionAttributeNames={"#n1": "a", "#n2": "b"},
    )["Item"] == {"a": {"b": 1}}
    # Missing or unused names in ExpressionAttributeNames are errors:
    with pytest.raises(ClientError, match="ValidationException"):
        test_table_s.get_item(
            ...
            ConsistentRead=True,
            ProjectionExpression="a.#n2",
            ExpressionAttributeNames={"#wrong": "b"},
        )
    with pytest.raises(ClientError, match="ValidationException"):
        test_table_s.get_item(
            Key={"p": p},
            ConsistentRead=True,
            ProjectionExpression="a.#n2",
            ExpressionAttributeNames={"#n2": "b", "#unused": "x"},
        )
...
def test_projection_expression_path_dot(test_table_s):
    ...
    assert test_table_s.get_item(
        ...
        ConsistentRead=True,
        ProjectionExpression="#name",
        ExpressionAttributeNames={"#name": "a.b"},
    )["Item"] == {"a.b": "hi"}
...
def test_projection_expression_path_overlap(test_table_s):
    ...
    for expr in [
        ...
        with pytest.raises(ClientError, match="ValidationException.* overlap"):
            print(expr)
            test_table_s.get_item(
                Key={"p": p}, ConsistentRead=True, ProjectionExpression=expr
            )
...
def test_query_projection_expression_path(test_table):
    p = random_string()
    items = [
        {"p": p, "c": str(i), "a": {"x": str(i * 10), "y": "hi"}, "b": "hello"}
        for i in range(10)
    ]
    with test_table.batch_writer() as batch:
        for item in items:
...
def test_scan_projection_expression_path(test_table):
    ...
    with test_table.batch_writer() as batch:
        for item in items:
            batch.put_item(item)
    got_items = [
        x for x in full_scan(test_table, ProjectionExpression="p, a.x") if x["p"] == p
    ]
...
def test_projection_expression_path_nesting_levels(test_table_s):
    ...
    test_table_s.get_item(
        Key={"p": p}, ConsistentRead=True, ProjectionExpression="a" + (".b" * 31)
    )
    ...
    with pytest.raises(ClientError, match="ValidationException.*nesting levels"):
        test_table_s.get_item(
            Key={"p": p}, ConsistentRead=True, ProjectionExpression="a" + (".b" * 32)
        )
