...
def test_basic_string_update_and_get(test_table):
    ...
    test_table.update_item(
        Key={"p": p, "c": c},
        AttributeUpdates={
            "attribute": {"Value": val, "Action": "PUT"},
            "another": {"Value": val2, "Action": "PUT"},
        },
    )
    item = test_table.get_item(Key={"p": p, "c": c}, ConsistentRead=True)["Item"]
    assert item["p"] == p
...
def test_empty_put_delete(test_table):
    ...
    test_table.update_item(
        Key={"p": p, "c": c}, AttributeUpdates={"hello": {"Action": "DELETE"}}
    )
    item = test_table.get_item(Key={"p": p, "c": c}, ConsistentRead=True)["Item"]
    assert item == {"p": p, "c": c}
...
def test_empty_update_delete(test_table):
    ...
    test_table.update_item(
        Key={"p": p, "c": c},
        AttributeUpdates={"hello": {"Value": "world", "Action": "PUT"}},
    )
    test_table.update_item(
        Key={"p": p, "c": c}, AttributeUpdates={"hello": {"Action": "DELETE"}}
    )
...
def test_update_bad_action(test_table):
    ...
    with pytest.raises(ClientError, match="ValidationException"):
        test_table.update_item(
            Key={"p": p, "c": c},
            AttributeUpdates={"attribute": {"Value": val, "Action": "NONEXISTENT"}},
        )
...
def test_basic_string_more_update(test_table):
    ...
    test_table.update_item(
        Key={"p": p, "c": c}, AttributeUpdates={"a1": {"Value": val1, "Action": "PUT"}}
    )
    test_table.update_item(
        Key={"p": p, "c": c}, AttributeUpdates={"a2": {"Value": val2, "Action": "PUT"}}
    )
    test_table.update_item(
        Key={"p": p, "c": c}, AttributeUpdates={"a1": {"Value": val3, "Action": "PUT"}}
    )
    test_table.update_item(
        Key={"p": p, "c": c}, AttributeUpdates={"a3": {"Action": "DELETE"}}
    )
    item = test_table.get_item(Key={"p": p, "c": c}, ConsistentRead=True)["Item"]
    assert item["p"] == p
    assert item["c"] == c
...
def test_item_operations_nonexistent_table(dynamodb):
    with pytest.raises(ClientError, match="ResourceNotFoundException"):
        dynamodb.meta.client.put_item(
            TableName="non_existent_table", Item={"a": {"S": "b"}}
        )
...
def test_item_operations_improper_named_table(dynamodb):
    with pytest.raises(ClientError, match="ValidationException"):
        dynamodb.meta.client.put_item(
            TableName="non!existent!table", Item={"a": {"S": "b"}}
        )
    with pytest.raises(ClientError, match="ValidationException"):
        dynamodb.meta.client.put_item(TableName="xx", Item={"a": {"S": "b"}})
...
def test_put_item_wrong_key_type(test_table):
    ...
    assert test_table.get_item(Key={"p": s, "c": s}, ConsistentRead=True)["Item"] == {
        "p": s,
        "c": s,
    }
...
def test_only_hash_key(test_table_s):
    ...
    assert test_table_s.get_item(Key={"p": s}, ConsistentRead=True)["Item"] == {
        "p": s,
        "hello": "world",
    }
    test_table_s.update_item(
        Key={"p": s}, AttributeUpdates={"hi": {"Value": "there", "Action": "PUT"}}
    )
    assert test_table_s.get_item(Key={"p": s}, ConsistentRead=True)["Item"] == {
        "p": s,
        "hello": "world",
...
def test_bytes_hash_key(test_table_b):
    ...
    for len in range(10, 18):
        ...
        assert test_table_b.get_item(Key={"p": p}, ConsistentRead=True)["Item"] == {
            "p": p,
            "attribute": val,
        }
...
def test_bytes_sort_key(test_table_sb):
    ...
    assert test_table_sb.get_item(Key={"p": p, "c": c}, ConsistentRead=True)[
        "Item"
    ] == {"p": p, "c": c, "attribute": val}
...
def test_large_blob_sort_key(test_table_sb):
    ...
    assert test_table_sb.get_item(Key={"p": s, "c": b}, ConsistentRead=True)[
        "Item"
    ] == {"p": s, "c": b}
...
def test_getitem_attributes_to_get(dynamodb, test_table):
    ...
    test_table.put_item(Item=item)
    for wanted in [
        ["a"],  # only non-key attribute
        ["c", "a"],  # a key attribute (sort key) and non-key
        ["p", "c"],  # entire key
        ["nonexistent"],  # Our item doesn't have this
    ]:
        got_item = test_table.get_item(
            Key={"p": p, "c": c}, AttributesToGet=wanted, ConsistentRead=True
        )["Item"]
...
def test_getitem_attributes_to_get_duplicate(dynamodb, test_table):
    ...
    with pytest.raises(ClientError, match="ValidationException.*Duplicate"):
        test_table.get_item(
            Key={"p": p, "c": c}, AttributesToGet=["a", "a"], ConsistentRead=True
        )
...
def test_getitem_attributes_to_get_empty(dynamodb, test_table):
    ...
    with pytest.raises(ClientError, match="ValidationException"):
        test_table.get_item(
            Key={"p": p, "c": c}, AttributesToGet=[], ConsistentRead=True
        )
...
def test_update_item_merge(test_table_s):
    ...
    test_table_s.update_item(
        Key={"p": p}, AttributeUpdates={"b": {"Value": "hello", "Action": "PUT"}}
    )
    assert test_table_s.get_item(Key={"p": p}, ConsistentRead=True)["Item"] == {
        "p": p,
        "a": "hi",
    ...
    test_table_s.update_item(
        Key={"p": p}, AttributeUpdates={"a": {"Value": "hey", "Action": "PUT"}}
    )
    assert test_table_s.get_item(Key={"p": p}, ConsistentRead=True)["Item"] == {
        "p": p,
        "a": "hey",
...
def test_update_item_non_existent(test_table_s):
    ...
    p = random_string()
    test_table_s.update_item(
        Key={"p": p}, AttributeUpdates={"a": {"Value": 3, "Action": "PUT"}}
    )
    assert test_table_s.get_item(Key={"p": p}, ConsistentRead=True)["Item"] == {
        "p": p,
        "a": 3,
...
def test_update_item_delete(test_table_s):
    ...
    for value in ["b", 3, bytearray("b", "utf-8"), True, False, None, [2, 3], {"a": 3}]:
    ...
    test_table_s.update_item(
        Key={"p": p}, AttributeUpdates={"a": {"Action": "DELETE", "Value": set([2, 4])}}
    )
    assert test_table_s.get_item(Key={"p": p}, ConsistentRead=True)["Item"] == {
        "p": p,
        "a": set([1, 3, 5]),
    ...
    with pytest.raises(ClientError, match="ValidationException.*empty"):
        test_table_s.update_item(
            Key={"p": p}, AttributeUpdates={"a": {"Action": "DELETE", "Value": set([])}}
        )
    ...
    test_table_s.update_item(
        Key={"p": p}, AttributeUpdates={"a": {"Action": "DELETE", "Value": set([2, 5])}}
    )
    assert test_table_s.get_item(Key={"p": p}, ConsistentRead=True)["Item"] == {
        "p": p,
        "a": set([1, 3]),
...
def test_update_item_add(test_table_s):
    ...
    test_table_s.update_item(
        Key={"p": p}, AttributeUpdates={"a": {"Action": "ADD", "Value": 2}}
    )
    assert test_table_s.get_item(Key={"p": p}, ConsistentRead=True)["Item"] == {
        "p": p,
        "a": 9,
    ...
    test_table_s.update_item(
        Key={"p": p}, AttributeUpdates={"a": {"Action": "ADD", "Value": Decimal(-3.5)}}
    )
    assert test_table_s.get_item(Key={"p": p}, ConsistentRead=True)["Item"] == {
        "p": p,
        "a": Decimal(5.5),
    ...
    test_table_s.update_item(
        Key={"p": p}, AttributeUpdates={"b": {"Action": "ADD", "Value": 2}}
    )
    assert test_table_s.get_item(Key={"p": p}, ConsistentRead=True)["Item"] == {
        "p": p,
        "a": Decimal(5.5),
    ...
    test_table_s.update_item(
        Key={"p": p}, AttributeUpdates={"a": {"Action": "ADD", "Value": set([3, 4])}}
    )
    assert test_table_s.get_item(Key={"p": p}, ConsistentRead=True)["Item"] == {
        "p": p,
        "a": set([1, 2, 3, 4]),
    ...
    test_table_s.update_item(
        Key={"p": p}, AttributeUpdates={"a": {"Action": "ADD", "Value": set([3, 5])}}
    )
    assert test_table_s.get_item(Key={"p": p}, ConsistentRead=True)["Item"] == {
        "p": p,
        "a": set([1, 2, 3, 4, 5]),
    ...
    test_table_s.update_item(
        Key={"p": p}, AttributeUpdates={"b": {"Action": "ADD", "Value": set([7])}}
    )
    test_table_s.update_item(
        Key={"p": p},
    ...
    test_table_s.update_item(
        Key={"p": p}, AttributeUpdates={"a": {"Action": "ADD", "Value": [3, 5]}}
    )
    assert test_table_s.get_item(Key={"p": p}, ConsistentRead=True)["Item"] == {
        "p": p,
        "a": [1, 2, 3, 4, 3, 5],
    ...
    test_table_s.update_item(
        Key={"p": p}, AttributeUpdates={"c": {"Action": "ADD", "Value": ["a", "b"]}}
    )
    assert test_table_s.get_item(Key={"p": p}, ConsistentRead=True)["Item"] == {
        "p": p,
        "a": [1, 2, 3, 4, 3, 5],
    ...
    test_table_s.update_item(
        Key={"p": p}, AttributeUpdates={"c": {"Action": "ADD", "Value": [3]}}
    )
    ...
    with pytest.raises(ClientError, match="ValidationException.*type"):
        ...
        test_table_s.update_item(
            Key={"p": p}, AttributeUpdates={"a": {"Action": "ADD", "Value": "s"}}
        )
    with pytest.raises(ClientError, match="ValidationException.*type"):
        test_table_s.put_item(Item={"p": p, "a": {"x": 1}})
        test_table_s.update_item(
            Key={"p": p}, AttributeUpdates={"a": {"Action": "ADD", "Value": {"y": 2}}}
        )
    with pytest.raises(ClientError, match="ValidationException.*type"):
        test_table_s.put_item(Item={"p": p, "a": set(["dog"])})
        test_table_s.update_item(
            Key={"p": p}, AttributeUpdates={"a": {"Action": "ADD", "Value": "cat"}}
        )
    ...
    with pytest.raises(ClientError, match="ValidationException.*mismatch"):
        test_table_s.update_item(
            Key={"p": p}, AttributeUpdates={"a": {"Action": "ADD", "Value": 3}}
        )
    test_table_s.put_item(Item={"p": p, "a": 2})
    with pytest.raises(ClientError, match="ValidationException.*mismatch"):
        test_table_s.update_item(
            Key={"p": p}, AttributeUpdates={"a": {"Action": "ADD", "Value": ["dog"]}}
        )
    with pytest.raises(ClientError, match="ValidationException.*mismatch"):
        test_table_s.update_item(
            Key={"p": p},
    ...
    for value in [3, set([1, 2]), set(["a", "b"]), [1, 2]]:
        test_table_s.delete_item(Key={"p": p})
        test_table_s.update_item(
            Key={"p": p}, AttributeUpdates={"a": {"Action": "ADD", "Value": value}}
        )
        assert test_table_s.get_item(Key={"p": p}, ConsistentRead=True)["Item"] == {
            "p": p,
            "a": value,
...
def test_update_item_empty_attribute(test_table_s):
    ...
    test_table_s.update_item(
        Key={"p": p}, AttributeUpdates={"e": {"Action": "PUT", "Value": {}}}
    )
    test_table_s.update_item(
        Key={"p": p}, AttributeUpdates={"f": {"Action": "PUT", "Value": ""}}
    )
    test_table_s.update_item(
        Key={"p": p},
...
def test_update_item_empty_key(
    test_table_s, test_table_b, test_table_ss, test_table_sb
):
    with pytest.raises(ClientError, match="ValidationException.*empty"):
        test_table_s.update_item(
            Key={"p": ""}, AttributeUpdates={"v": {"Action": "PUT", "Value": "abc"}}
        )
    with pytest.raises(ClientError, match="ValidationException.*empty"):
        test_table_b.update_item(
            Key={"p": bytearray("", "utf-8")},
            AttributeUpdates={"v": {"Action": "PUT", "Value": "abc"}},
        )
    with pytest.raises(ClientError, match="ValidationException.*empty"):
        test_table_ss.update_item(
            Key={"p": "abc", "c": ""},
            AttributeUpdates={"v": {"Action": "PUT", "Value": "abc"}},
        )
    with pytest.raises(ClientError, match="ValidationException.*empty"):
        test_table_sb.update_item(
            Key={"p": "abc", "c": bytearray("", "utf-8")},
            AttributeUpdates={"v": {"Action": "PUT", "Value": "abc"}},
        )
...
def test_put_item_empty_key(test_table_s, test_table_b, test_table_ss, test_table_sb):
    ...
    with pytest.raises(ClientError, match="ValidationException.*empty"):
        test_table_sb.put_item(
            Item={"p": "abc", "c": bytearray("", "utf-8"), "v": "something"}
        )
...
def test_unused_entries_no_expression(test_table_s):
    ...
    # PutItem:
    with pytest.raises(
        ClientError, match="ValidationException.*ExpressionAttributeNames"
    ):
        test_table_s.put_item(
            Item={"p": p, "a": "dog"}, ExpressionAttributeNames={"#name1": "x"}
        )
    with pytest.raises(
        ClientError, match="ValidationException.*ExpressionAttributeValues"
    ):
        test_table_s.put_item(
            Item={"p": p, "a": "dog"}, ExpressionAttributeValues={":val1": 1}
        )
    # DeleteItem:
    with pytest.raises(
        ClientError, match="ValidationException.*ExpressionAttributeNames"
    ):
        test_table_s.delete_item(Key={"p": p}, ExpressionAttributeNames={"#name1": "x"})
    with pytest.raises(
        ClientError, match="ValidationException.*ExpressionAttributeValues"
    ):
        test_table_s.delete_item(Key={"p": p}, ExpressionAttributeValues={":val1": 1})
    # UpdateItem:
    with pytest.raises(
        ClientError, match="ValidationException.*ExpressionAttributeNames"
    ):
    ...
    with pytest.raises(
        ...
        test_table_s.get_item(Key={"p": p}, ExpressionAttributeNames={"#name1": "x"})
