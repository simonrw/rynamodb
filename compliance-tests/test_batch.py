...
def test_basic_batch_write_item(test_table):
    ...
    for i in range(count):
        item = test_table.get_item(
            Key={"p": "batch{}".format(i), "c": "batch_ck{}".format(i)},
            ConsistentRead=True,
        )["Item"]
...
def test_batch_write_nonduplicate_multiple_tables(test_table_s, test_table_s_2):
    ...
    assert test_table_s_2.get_item(Key={"p": p}, ConsistentRead=True)["Item"] == {
        "p": p,
        "b": "hello",
    }
...
def test_batch_write_multiple_tables(test_table_s, test_table):
    ...
    assert test_table.get_item(Key={"p": p1, "c": c1}, ConsistentRead=True)["Item"] == {
        "p": p1,
        "c": c1,
        "a": "hi",
    }
    assert test_table_s.get_item(Key={"p": p2}, ConsistentRead=True)["Item"] == {
        "p": p2,
        "b": "hello",
    }
...
def test_batch_get_item(test_table):
    ...
    reply = test_table.meta.client.batch_get_item(
        RequestItems={test_table.name: {"Keys": keys, "ConsistentRead": True}}
    )
...
def test_batch_get_item_same_partition_key(test_table):
    ...
    items_half = items[::2]
    reply = test_table.meta.client.batch_get_item(
        RequestItems={test_table.name: {"Keys": keys_half, "ConsistentRead": True}}
    )
    got_items = reply["Responses"][test_table.name]
...
def test_batch_get_item_hash(test_table_s):
    ...
    reply = test_table_s.meta.client.batch_get_item(
        RequestItems={test_table_s.name: {"Keys": keys, "ConsistentRead": True}}
    )
...
def test_batch_get_item_missing(test_table_s):
    ...
    reply = test_table_s.meta.client.batch_get_item(
        RequestItems={
            test_table_s.name: {
                "Keys": [{"p": random_string()}, {"p": random_string()}, {"p": p}],
                "ConsistentRead": True,
            }
        }
    )
...
def test_batch_unprocessed(test_table_s):
    p = random_string()
    write_reply = test_table_s.meta.client.batch_write_item(
        RequestItems={
            test_table_s.name: [{"PutRequest": {"Item": {"p": p, "a": "hi"}}}],
        }
    )
    assert (
        "UnprocessedItems" in write_reply and write_reply["UnprocessedItems"] == dict()
    )

    read_reply = test_table_s.meta.client.batch_get_item(
        RequestItems={
            test_table_s.name: {
                "Keys": [{"p": p}],
                "ProjectionExpression": "p, a",
                "ConsistentRead": True,
            }
        }
    )
...
def test_batch_get_item_duplicate(test_table, test_table_s):
    ...
    with pytest.raises(ClientError, match="ValidationException.*duplicates"):
        test_table_s.meta.client.batch_get_item(
            RequestItems={test_table_s.name: {"Keys": [{"p": p}, {"p": p}]}}
        )
    c = random_string()
    with pytest.raises(ClientError, match="ValidationException.*duplicates"):
        test_table.meta.client.batch_get_item(
            RequestItems={
                test_table.name: {"Keys": [{"p": p, "c": c}, {"p": p, "c": c}]}
            }
        )
    # Not a duplicate:
    c2 = random_string()
...
def test_batch_write_item_large(test_table_sn):
    ...
    write_reply = test_table_sn.meta.client.batch_write_item(
        RequestItems={
            test_table_sn.name: [
                {"PutRequest": {"Item": {"p": p, "c": i, "content": long_content}}}
                for i in range(25)
            ],
        }
    )
...
def test_batch_write_item_too_many(test_table_sn):
    ...
    with pytest.raises(ClientError, match="ValidationException.*length"):
        test_table_sn.meta.client.batch_write_item(
            RequestItems={
                test_table_sn.name: [
                    {"PutRequest": {"Item": {"p": p, "c": i}}} for i in range(30)
                ]
            }
        )
    with pytest.raises(ClientError, match="ValidationException.*length"):
        test_table_sn.meta.client.batch_write_item(
            RequestItems={
                test_table_sn.name: [
                    {"DeleteRequest": {"Key": {"p": p, "c": i}}} for i in range(30)
                ]
            }
        )
...
def test_batch_get_item_partial(scylla_only, dynamodb, rest_api, test_table_sn):
    ...
    with test_table_sn.batch_writer() as batch:
        for i in range(count):
            batch.put_item(
                Item={"p": p + str(i % partitions), "c": i, "content": content}
            )
    responses = []
    to_read = {
        test_table_sn.name: {
            "Keys": [{"p": p + str(c % partitions), "c": c} for c in range(count)],
            "ConsistentRead": True,
        }
    }
