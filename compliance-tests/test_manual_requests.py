...
def test_too_large_request_chunked(dynamodb, test_table):
    if Version(urllib3.__version__) < Version("1.26"):
        pytest.skip(
            "urllib3 before 1.26.0 threw broken pipe and did not read response and cause issue #8195. Fixed by pull request urllib3/urllib3#1524"
        )
    # To make a request very large, we just stuff it with a lot of spaces :-)
    spaces = " " * (17 * 1024 * 1024)
    req = get_signed_request(
        dynamodb,
        "PutItem",
        '{"TableName": "'
        + test_table.name
    ...
    response = requests.post(
        req.url, headers=req.headers, data=generator(req.body), verify=False
    )
...
@pytest.mark.xfail(
    reason="issue #12166. Note that only fails very rairly, will usually xpass"
)
...
def test_incorrect_json(dynamodb, test_table):
    ...
    validate_resp = (
        lambda t: "SerializationException" in t
        or "ValidationException" in t
        or "Page Not Found" in t
    )
    for i in range(len(correct_req)):
        req = get_signed_request(dynamodb, "PutItem", correct_req[:i])
        response = requests.post(
            req.url, headers=req.headers, data=req.body, verify=False
        )
...
def test_tags_return_empty_body(dynamodb, test_table):
    ...
    req = get_signed_request(
        dynamodb, "UntagResource", '{"ResourceArn": "' + arn + '", "TagKeys": ["k"]}'
    )
    response = requests.post(req.url, headers=req.headers, data=req.body, verify=False)
    assert not response.text
...
def test_exception_escape_raw(dynamodb, test_table_s):
    payload = (
        '{"TableName": "'
        + test_table_s.name
        + '", "Key": {"p": {"S": "hello"}}, "UpdateExpression": "ADD n :inc", "ExpressionAttributeValues": {":inc": {"S": "1"}}}'
    )
...
def put_item_binary_data_in_key(dynamodb, test_table_b, item_data):
    payload = '{"TableName": "%s", "Item": {"p": {"B": "%s"}}}' % (
        test_table_b.name,
        item_data,
    )
    req = get_signed_request(dynamodb, "PutItem", payload)
...
def scan_with_binary_data_in_cond_expr(
    dynamodb, test_table_b, filter_expr, expr_attr_values
):
    payload = """{
        "TableName": "%s",
        "FilterExpression": "%s",
