...
def clear_write_isolation(table):
    ...
    table.meta.client.untag_resource(
        ResourceArn=arn, TagKeys=["system:write_isolation"]
    )
...
def test_update_condition_eq_success(test_table_s):
    ...
    for val in values:
        ...
        assert (
            test_table_s.get_item(Key={"p": p}, ConsistentRead=True)["Item"]["c"] == i
        )
        test_table_s.update_item(
            Key={"p": p},
        ...
        assert (
            test_table_s.get_item(Key={"p": p}, ConsistentRead=True)["Item"]["d"] == i
        )
...
def test_update_condition_eq_different(test_table_s):
    ...
    for val1 in values:
        test_table_s.update_item(
            Key={"p": p}, AttributeUpdates={"a": {"Value": val1, "Action": "PUT"}}
        )
        for val2 in values:
            ...
            if (val1 is True and val2 is True) or (
                ...
                with pytest.raises(
                    ClientError, match="ConditionalCheckFailedException"
                ):
                    test_table_s.update_item(
                        Key={"p": p},
                        UpdateExpression="SET a = :val1",
...
def test_update_condition_eq_set(test_table_s):
    ...
    test_table_s.update_item(
        ...
        ExpressionAttributeValues={
            ":val1": 3,
            ":oldval": set(["chinchilla", "cat", "dog", "mouse"]),
        },
    )
...
def test_update_condition_ne_set(test_table_s):
    ...
    test_table_s.update_item(
        ...
        ExpressionAttributeValues={
            ":val1": 3,
            ":oldval": set(["chinchilla", "cat", "dog", "horse"]),
        },
    )
    assert "b" in test_table_s.get_item(Key={"p": p}, ConsistentRead=True)["Item"]
...
def test_update_condition_lt(test_table_s):
    ...
    test_table_s.update_item(
        Key={"p": p}, AttributeUpdates={"x": {"Value": [1, 2, 3], "Action": "PUT"}}
    )
    with pytest.raises(ClientError, match="ConditionalCheckFailedException"):
        test_table_s.update_item(
            Key={"p": p},
...
def test_update_condition_between(test_table_s):
    ...
    test_table_s.update_item(
        Key={"p": p}, AttributeUpdates={"two": {"Value": 2, "Action": "PUT"}}
    )
    with pytest.raises(ClientError, match="ConditionalCheckFailedException"):
        test_table_s.update_item(
            Key={"p": p},
...
def test_update_condition_in(test_table_s):
    ...
    test_table_s.update_item(
        Key={"p": p}, AttributeUpdates={"a": {"Value": 74, "Action": "PUT"}}
    )
    ...
    test_table_s.update_item(
        Key={"p": p}, AttributeUpdates={"a": {"Value": 174, "Action": "PUT"}}
    )
    with pytest.raises(ClientError, match="ConditionalCheckFailedException.*"):
        test_table_s.update_item(
            Key={"p": p},
...
def test_update_condition_attribute_type(test_table_s):
    ...
    updates = {
        "a{}".format(i): {"Value": type_values[i][1], "Action": "PUT"}
...
def test_update_condition_other_funcs(test_table_s):
    p = random_string()
    test_table_s.update_item(
        Key={"p": p}, AttributeUpdates={"a": {"Value": "hello", "Action": "PUT"}}
    )
...
def test_update_condition_attribute_reference(test_table_s):
    p = random_string()
    test_table_s.update_item(
        Key={"p": p}, AttributeUpdates={"and": {"Value": 1, "Action": "PUT"}}
    )
    test_table_s.update_item(
        Key={"p": p},
        UpdateExpression="SET c = :val",
...
def test_update_condition_nested_attribute_reference(test_table_s):
    p = random_string()
    test_table_s.update_item(
        Key={"p": p}, AttributeUpdates={"and": {"Value": {"or": 2}, "Action": "PUT"}}
    )
    test_table_s.update_item(
        Key={"p": p},
        UpdateExpression="SET c = :val",
...
def test_update_condition_unused_entries_succeeded(test_table_s):
    p = random_string()
    test_table_s.update_item(
        Key={"p": p}, AttributeUpdates={"b": {"Value": 2, "Action": "PUT"}}
    )
    # unused val3:
    with pytest.raises(ClientError, match="ValidationException.*val3"):
        test_table_s.update_item(
...
def test_condition_expression_unsigned_bytes(test_table_s):
    ...
    test_table_s.update_item(
        ...
        ExpressionAttributeValues={
            ":newval": 3,
            ":oldval1": bytearray([126]),
            ":oldval2": bytearray([128]),
        },
    )
    assert test_table_s.get_item(Key={"p": p}, ConsistentRead=True)["Item"]["z"] == 3
...
def test_update_item_condition_key_ne(test_table_s):
    ...
    test_table_s.update_item(
        Key={"p": p}, ConditionExpression="p <> :p", ExpressionAttributeValues={":p": p}
    )
...
def test_update_item_condition_key_attribute_not_exists(test_table_s):
    ...
    test_table_s.update_item(
        Key={"p": p}, ConditionExpression="attribute_not_exists(p)"
    )
    assert "Item" in test_table_s.get_item(Key={"p": p}, ConsistentRead=True)
