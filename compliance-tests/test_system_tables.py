...
def test_fetch_from_system_tables(scylla_only, dynamodb):
    ...
    tables_response = client.scan(
        TableName=internal_prefix + "system_schema.tables",
    ...
    for item in tables_response["Items"]:
        ...
        col_response = client.query(
            TableName=internal_prefix + "system_schema.columns",
        ...
        key_columns = [
            ...
            if item["kind"] == "clustering" or item["kind"] == "partition_key"
        ]
        ...
        response = client.scan(
            TableName=qualified_name, AttributesToGet=key_columns, Limit=50
        )
        print(ks_name, table_name, len(str(response)), time.time() - start)


def test_block_access_to_non_system_tables_with_virtual_interface(
    scylla_only, test_table_s, dynamodb
):
    client = dynamodb.meta.client
    with pytest.raises(
    ...
    for wrong_name_postfix in ["", "a", "xxx", "system_auth.roles", "table_name"]:
        with pytest.raises(ClientError, match=internal_prefix):
            dynamodb.create_table(
                TableName=internal_prefix + wrong_name_postfix,
