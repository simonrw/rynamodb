...
def test_ttl_disable(dynamodb, veryslow_on_aws):
    with new_test_table(
        ...
        assert client.describe_time_to_live(TableName=table.name)[
        ...
        with table.batch_writer() as batch:
            for p in range(N):
                for c in range(100):
                    ...
                    expiration = (
                        int(time.time()) - 60 if c == 0 else int(time.time()) + 3600
                    )
                    batch.put_item(Item={"p": p, "c": c, "expiration": expiration})
        start_time = time.time()
        while time.time() < start_time + max_duration:
            count = 0
