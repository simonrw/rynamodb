...
def test_gsi_empty_value_with_range_key(test_table_gsi_3):
    with pytest.raises(ClientError, match="ValidationException.*empty"):
        test_table_gsi_3.put_item(
            Item={"p": random_string(), "a": "", "b": random_string()}
        )
    with pytest.raises(ClientError, match="ValidationException.*empty"):
        test_table_gsi_3.put_item(
            Item={"p": random_string(), "a": random_string(), "b": ""}
        )
...
def test_gsi_null_value_with_range_key(test_table_gsi_3):
    with pytest.raises(ClientError, match="ValidationException.*NULL"):
        test_table_gsi_3.put_item(
            Item={"p": random_string(), "a": None, "b": random_string()}
        )
    with pytest.raises(ClientError, match="ValidationException.*NULL"):
        test_table_gsi_3.put_item(
            Item={"p": random_string(), "a": random_string(), "b": None}
        )
