...
def test_filter_expression_between_bad_type(test_table_sn_with_data):
    ...
    got_items = full_query(
        table,
        KeyConditionExpression="p=:p",
        FilterExpression="s between :xv and l",
    ...
    got_items = full_query(
        table,
        KeyConditionExpression="p=:p",
        FilterExpression="s between l and :xv",
        ExpressionAttributeValues={":p": p, ":xv": "cat"},
    )
    assert got_items == []
    got_items = full_query(
        table,
        KeyConditionExpression="p=:p",
        FilterExpression="s between i and :xv",
        ExpressionAttributeValues={":p": p, ":xv": "cat"},
    )
    assert got_items == []
...
def test_filter_expression_between(test_table_sn_with_data):
    ...
    for xn in ["i", "s", "b"]:
        ...
        got_items = full_query(
            table,
            KeyConditionExpression="p=:p",
            FilterExpression=(xn + " BetWEeN :xv1 AnD :xv2"),
...
def test_filter_expression_num_between_reverse(test_table_sn_with_data):
    ...
    with pytest.raises(ClientError, match="ValidationException.* BETWEEN"):
        full_query(
            table,
            KeyConditionExpression="p=:p",
            FilterExpression="i BetWEeN :i1 AnD :i2",
...
def test_filter_expression_in(test_table_sn_with_data):
    ...
    for xn in ["i", "s", "b"]:
        ...
        got_items = full_query(
            table,
            KeyConditionExpression="p=:p",
            FilterExpression=(xn + " In (:xv1, :xv2, :xv3)"),
...
def test_filter_expression_num_begins(test_table_sn_with_data):
    ...
    with pytest.raises(ClientError, match="ValidationException.*begins_with"):
        full_query(
            table,
            KeyConditionExpression="p=:p",
            FilterExpression="begins_with(i, :i)",
            ExpressionAttributeValues={":p": p, ":i": i},
        )
...
def test_filter_expression_string_begins(test_table_sn_with_data):
    ...
    got_items = full_query(
        table,
        KeyConditionExpression="p=:p",
        FilterExpression="begins_with(s, :start)",
...
def test_filter_expression_bytes_begins(test_table_sn_with_data):
    ...
    got_items = full_query(
        table,
        KeyConditionExpression="p=:p",
        FilterExpression="begins_with(b, :start)",
        ExpressionAttributeValues={":p": p, ":start": start},
    )
...
def test_filter_expression_num_contains(test_table_sn_with_data):
    ...
    for xn in ["ns", "l"]:
        ...
        got_items = full_query(
            table,
            KeyConditionExpression="p=:p",
            FilterExpression="contains(" + xn + ", :i)",
...
def test_filter_expression_set_contains(test_table_sn_with_data):
    ...
    got_items = full_query(
        table,
        KeyConditionExpression="p=:p",
        FilterExpression="contains(ns, :s)",
...
def test_filter_expression_string_contains(test_table_sn_with_data):
    ...
    got_items = full_query(
        table,
        KeyConditionExpression="p=:p",
        FilterExpression="contains(s, :substring)",
    ...
    got_items = full_query(
        table,
        KeyConditionExpression="p=:p",
        FilterExpression="contains(ss, :s)",
        ExpressionAttributeValues={":p": p, ":s": s},
    )
...
def test_filter_expression_bytes_contains(test_table_sn_with_data):
    ...
    got_items = full_query(
        table,
        KeyConditionExpression="p=:p",
        FilterExpression="contains(b, :substring)",
        ExpressionAttributeValues={":p": p, ":substring": substring},
    )
...
def test_filter_expression_bool_contains(test_table_sn_with_data):
    ...
    got_items = full_query(
        table,
        KeyConditionExpression="p=:p",
        FilterExpression="contains(bool, :i)",
...
def test_filter_expression_attribute_type(test_table_sn_with_data):
    ...
    got_items = full_query(
        table,
        KeyConditionExpression="p=:p",
        FilterExpression="attribute_type(r, :t)",
        ExpressionAttributeValues={":p": p, ":t": "S"},
    )
    expected_items = [
        item for item in items if "r" in item and isinstance(item["r"], str)
    ]
    assert got_items == expected_items
    ...
    expected_items = [
        item for item in items if "r" in item and isinstance(item["r"], int)
    ]
    assert got_items == expected_items
    ...
    expected_items = [
        item for item in items if "r" in item and isinstance(item["r"], bytearray)
    ]
    assert got_items == expected_items
    ...
    got_items = full_query(
        table,
        KeyConditionExpression="p=:p",
        FilterExpression="attribute_type(ns, :t)",
    ...
    got_items = full_query(
        table,
        KeyConditionExpression="p=:p",
        FilterExpression="attribute_type(bool, :t)",
    ...
    got_items = full_query(
        table,
        KeyConditionExpression="p=:p",
        FilterExpression="attribute_type(m, :t)",
    ...
    got_items = full_query(
        table,
        KeyConditionExpression="p=:p",
        FilterExpression="attribute_type(l, :t)",
...
def test_filter_expression_string_size(test_table_sn_with_data):
    ...
    got_items = full_query(
        table,
        KeyConditionExpression="p=:p",
        FilterExpression="size(s) BETWEEN :i1 AND :i2",
        ExpressionAttributeValues={":p": p, ":i1": len1, ":i2": len2},
    )
    expected_items = [
        item for item in items if len(item["s"]) >= len1 and len(item["s"]) <= len2
    ]
    assert got_items == expected_items
    got_items = full_query(
        table,
        KeyConditionExpression="p=:p",
        FilterExpression="size(s) IN (:i1, :i2)",
        ExpressionAttributeValues={":p": p, ":i1": len1, ":i2": len2},
    )
    expected_items = [
        item for item in items if len(item["s"]) == len1 or len(item["s"]) == len2
    ]
    assert got_items == expected_items
...
def test_filter_expression_or(test_table_sn_with_data):
    ...
    expected_items = [
        item for item in items if item["i"] == i1 or item["i"] == i2 or item["i"] == i3
    ]
    assert got_items == expected_items
...
def test_filter_expression_complex(test_table_sn_with_data):
    ...
    got_items = full_query(
        table,
        KeyConditionExpression="p=:p",
        FilterExpression="(((i=:i1) AND s=:s1) OR i=:i2 oR i=:i3) or NoT i<:i4",
    ...
    expected_items = [
        item
        for item in items
        if item["i"] == i1 or item["i"] == i2 or item["i"] == i3 or item["i"] >= i4
    ]
    assert got_items == expected_items
...
def test_filter_expression_precedence(test_table_sn_with_data):
    ...
    got_items = full_query(
        table,
        KeyConditionExpression="p=:p",
        FilterExpression="i=:i2 OR i=:i1 AND s=:s1",
...
def test_filter_expression_compare_attributes(test_table_sn_with_data):
    ...
    got_items = full_query(
        table,
        KeyConditionExpression="p=:p",
        FilterExpression="i in (j, :k)",
        ExpressionAttributeValues={":p": p, ":k": 3},
    )
    expected_items = [
        item
        for item in items
        if item["i"] == 3 or ("j" in item and item["i"] == item["j"])
    ]
    assert got_items == expected_items
...
def test_filter_expression_and_select_count(test_table):
    ...
    test_table.put_item(Item={"p": p, "c": "hi", "x": "dog", "y": "cat"})
    test_table.put_item(Item={"p": p, "c": "yo", "x": "mouse", "y": "horse"})
    (prefilter_count, postfilter_count, pages, got_items) = full_query_and_counts(
        test_table,
        KeyConditionExpression="p=:p",
        FilterExpression="x=:x",
        Select="COUNT",
...
def test_filter_expression_and_projection_expression_nested(test_table):
    ...
    test_table.put_item(
        Item={"p": p, "c": "yo", "x": {"a": "mouse", "b": 4}, "y": "horse"}
    )
...
def test_filter_expression_scan(filled_test_table):
    ...
    expected_items = [
        item for item in items if "attribute" in item and item["attribute"] == "xxxx"
    ]
...
def test_filter_expression_scan_partition_key(filled_test_table):
    table, items = filled_test_table
    got_items = full_scan(
        table, FilterExpression="p=:a", ExpressionAttributeValues={":a": "3"}
    )
...
def test_filter_expression_scan_sort_key(filled_test_table):
    table, items = filled_test_table
    got_items = full_scan(
        table, FilterExpression="c=:a", ExpressionAttributeValues={":a": 3}
    )
