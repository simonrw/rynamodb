...
def compare_events(expected_events, output, mode):
    ...
    for event in output:
        ...
        if (
            expected_type != "*"
        ):  # hack to allow a caller to not test op, to bypass issue #6918.
...
def test_streams_updateitem_old_image_empty_item(
    test_table_ss_old_image, dynamodbstreams
):
    def do_updates(table, p, c):
        events = []
...
def test_streams_updateitem_old_image_lsi(
    test_table_ss_old_image_and_lsi, dynamodbstreams
):
    def do_updates(table, p, c):
        events = []
        table.update_item(
        ...
        # test_streams_updateitem_keys_only (issue #6918).
        events.append(
            ["*", {"p": p, "c": c}, None, {"p": p, "c": c, "x": 2, "k": "dog"}]
        )
        table.update_item(
            Key={"p": p, "c": c},
...
def test_streams_1_new_and_old_images(
    test_table_ss_new_and_old_images, dynamodbstreams
):
    do_test(
        test_table_ss_new_and_old_images,
        dynamodbstreams,
