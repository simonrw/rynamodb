...
def test_query_filtering_attributes_equality(filled_test_table):
    ...
    query_filter = {
        "attribute": {"AttributeValueList": ["xxxx"], "ComparisonOperator": "EQ"}
    }
    got_items = full_query(
        test_table,
        KeyConditions={
    ...
    print(got_items)
    assert multiset(
        [item for item in items if item["p"] == "long" and item["attribute"] == "xxxx"]
    ) == multiset(got_items)

    query_filter = {
        "attribute": {"AttributeValueList": ["xxxx"], "ComparisonOperator": "EQ"},
        "another": {"AttributeValueList": ["yy"], "ComparisonOperator": "EQ"},
    }
...
def test_query_select(test_table_sn):
    ...
    p = random_string()
    items = [
        {"p": p, "c": num, "x": num} for num in random.sample(numbers, len(numbers))
    ]
    with test_table_sn.batch_writer() as batch:
        for item in items:
            batch.put_item(item)
