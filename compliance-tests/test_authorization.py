...
def test_wrong_key_access(request, dynamodb):
    print(
        "Please make sure authorization is enforced in your Scylla installation: alternator_enforce_authorization: true"
    )
    ...
    with pytest.raises(ClientError, match="UnrecognizedClientException"):
        if url.endswith(".amazonaws.com"):
            boto3.client(
                ...
                aws_secret_access_key="wrong_key",
            ).describe_endpoints()
        else:
            verify = not url.startswith("https")
            boto3.client(
...
def test_expired_signature(dynamodb, test_table):
    ...
    assert (
        "InvalidSignatureException" in response.text
        and "Signature expired" in response.text
    )
...
def test_signature_too_futuristic(dynamodb, test_table):
    ...
    assert (
        "InvalidSignatureException" in response.text
        and "Signature not yet current" in response.text
    )
